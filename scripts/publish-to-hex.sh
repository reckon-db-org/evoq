...
# version src/evoq.app.src happened to name.
#
...
# unreviewed. That mattered especially here, because evoq's gate is new and
...
EVOQ_VERSION="$(sed -n 's/.*{vsn, *"\([^"]*\)"}.*/\1/p' src/evoq.app.src | head -n 1)"

echo
echo "==> evoq ${EVOQ_VERSION}: building the package and docs"
rebar3 hex build

# evoq ships no NIF, so the docs build compiling the tree is the whole native
...
==> evoq ${EVOQ_VERSION} is ready to release. Nothing was published.

To release it:
  git push origin v${EVOQ_VERSION}
...
Afterwards, to check hex serves the tagged code rather than assuming it:
  scripts/is_hex_serving_what_git_says.sh ${EVOQ_VERSION}
EOF
